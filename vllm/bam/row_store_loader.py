...
def import_bam_kv_store():
    """按本地实验环境的常见路径探测并导入 BaMKVStore。

    KV fast path 第一阶段仍复用 BaM_IOStack/gids_module 下的 Python 薄封装。
    单独提供 loader 的原因是：vLLM 侧不应该假设 BaM_IOStack 一定被安装成
    site-package；它和 `bam_row_store.py` 一样，优先从本地实验目录加载。
    """
    errors: List[str] = []
    for path in _candidate_import_paths():
        path_str = str(path)
        if path.exists() and path_str not in sys.path:
            sys.path.insert(0, path_str)
        try:
            module = importlib.import_module("bam_kv_store")
            return module.BaMKVStore
        except Exception as exc:  # pragma: no cover - 这里只做环境探测
            errors.append(f"{path_str}: {type(exc).__name__}: {exc}")
    ...
    raise ImportError("Failed to import bam_kv_store. Tried paths:\n" +
                      "\n".join(errors))
