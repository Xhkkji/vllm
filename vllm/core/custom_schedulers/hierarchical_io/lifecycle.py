...
"""层级 restore 父事务与 window 子请求的生命周期。"""
...
from .plan import LayerRestorePlan, LayerWindow
...
class HierarchicalRestoreProgress:
    ...
    plan_id: str
    window: LayerWindow
    first_window_became_ready: bool
    first_window_ready: bool
    all_terminal: bool
    all_ready: bool
    failed: bool
    cancelled: bool
    first_window_ready_monotonic_ns: Optional[int]
    plan_created_monotonic_ns: int


@dataclass
class _RestoreState:
    plan: LayerRestorePlan
    request_ids: Tuple[str, ...]
    terminal_request_ids: set[str]
    ready_request_ids: set[str]
    failed: bool = False
    cancelled: bool = False
    first_window_ready_monotonic_ns: Optional[int] = None


class HierarchicalRestoreController:
    """聚合多个 window completion，但不管理 block/sequence 所有权。
    ...
    def register(self, plan: LayerRestorePlan,
        ...
        if len(ids) != len(plan.windows) or len(set(ids)) != len(ids):
            raise ValueError("one unique request_id is required per window")
    ...
    def mark_ready(
        ...
        state, window = self._mark_terminal(request_id)
        state.ready_request_ids.add(request_id)
        became_ready = False
        if (window.index == 0
                and state.first_window_ready_monotonic_ns is None):
            state.first_window_ready_monotonic_ns = (
                time.monotonic_ns()
                if now_monotonic_ns is None else now_monotonic_ns)
            became_ready = True
        return self._snapshot(state, window, became_ready)

    def mark_error(self, request_id: str) -> HierarchicalRestoreProgress:
        state, window = self._mark_terminal(request_id)
        state.failed = True
        return self._snapshot(state, window, False)

    def cancel_plan(self, plan_id: str) -> None:
        self._states[plan_id].cancelled = True

    def release(self, plan_id: str) -> LayerRestorePlan:
    ...
    def _mark_terminal(self,
                       request_id: str) -> Tuple[_RestoreState, LayerWindow]:
        plan_id, window_index = self._request_to_plan[request_id]
        state = self._states[plan_id]
        ...
        return state, state.plan.windows[window_index]
    ...
    def _snapshot(
        ...
        return HierarchicalRestoreProgress(
            ...
            window=window,
            first_window_became_ready=first_window_became_ready,
            first_window_ready=(state.first_window_ready_monotonic_ns
                                is not None),
            all_terminal=all_terminal,
            all_ready=all_ready,
            failed=state.failed,
            cancelled=state.cancelled,
            first_window_ready_monotonic_ns=(
                state.first_window_ready_monotonic_ns),
