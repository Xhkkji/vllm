...
class BaMKVLayout:
    ...
    def from_gpu_cache(cls, gpu_cache: List[torch.Tensor]) -> "BaMKVLayout":
        ...
        row_bytes = int(sample_layer[:, 0, :].numel() * sample_layer.element_size())
    ...
    def row_offset(self, cpu_block_id: int, layer_id: int) -> int:
        return int(cpu_block_id) * self.num_layers + int(layer_id)
